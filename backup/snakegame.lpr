...
const
  Width = 800;                   // Ширина графического окна
  Height = 600;                  // Высота графического окна
  CellSize = 20;                 // Размер клетки в пикселях
  MaxTopScores = 5;              // Максимальное количество рекордов
  BonusDuration = 7000;          // Время жизни бонусной еды (мс)
  ScoreFile = 'highscores.txt'; // Файл для сохранения очков

{ ======================== Типы данных ======================== }

type
  PNode = ^TNode;                // Указатель на сегмент змейки
  TNode = record
    x, y: Integer;               // Координаты сегмента
    next: PNode;                 // Следующий сегмент
  end;

  TFood = record
    x, y: Integer;               // Координаты еды
    isBonus: Boolean;           // Флаг — бонусная или обычная
    createdAt: LongInt;         // Время появления бонуса
  end;

  TScore = record
    Points: Integer;            // Кол-во очков
  end;

{ ======================== Глобальные переменные ======================== }

var
  gd, gm: SmallInt;              // Графические параметры
  SnakeHead, SnakeTail: PNode;  // Указатели на голову и хвост змейки
  dx, dy: Integer;               // Направление движения
  Food: TFood;                   // Текущая еда
  Score, Level, DelayTime: Integer; // Счёт, уровень, задержка
  GameOver: Boolean;            // Флаг окончания игры
  HighScores: array[1..MaxTopScores] of TScore; // Таблица рекордов

{ ======================== Системные процедуры ======================== }

procedure HideConsole;
begin
  ShowWindow(GetConsoleWindow, SW_HIDE);  // Скрыть консольное окно
end;
...
begin
  Level := Score div 50 + 1;       // Новый уровень каждые 50 очков
  DelayTime := 150 - Level * 10;   // Уменьшаем задержку (ускоряем)
  if
    DelayTime < 40
  then
    DelayTime := 40;
...
begin
  ...
  repeat
    ...
    then
      ...
      then
        begin
          if Food.isBonus then Inc(Score, 30)
          else Inc(Score, 10);
          GenerateFood;
          UpdateLevel;
        end
      ...
      if Food.isBonus and (GetTickCount - Food.createdAt > BonusDuration) then
      GenerateFood;
